...
Ou, se o pacote estiver no PYTHONPATH:
    ...
    import pymol_vitral; pymol_vitral.load()
"""
...
def load(auto=True):
    ...
    from pymol_vitral import core, membrane, protein
    ...
    print("[vitral] v%s carregado de %s" % (__version__, os.path.dirname(os.path.abspath(__file__))))
    print("[vitral] membrana: preset_memb1..10, memb_color, memb_water")
    print("[vitral] proteina: preset_prot1..10, prot_color, prot_water, "
          "prot_ions")
    print("[vitral] comum:    mv_ao, mv_shadows, mv_realism, mv_paper, "
    ...
    if not cmd.get_object_list("all"):
        print("[vitral] sessao vazia. Carregue uma estrutura e rode "
